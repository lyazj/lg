...
public:
  GLBezierSurfaceBuilder(const vec3 *c, vector<vec3> &v, vector<GLuint> &e) : controls(c), vertices(v), elements(e) { }
...
void GLBezierSurfaceBuilder::Build(GLint division)
{
  ...
  if(division) Build(0, 0, division);

  elements.clear();
  elements.reserve(2 * n * (n + 1));
  for(GLuint istack = 0; istack < n; ++istack) {
    for(GLuint islice = 0; islice <= n; ++islice) {
      elements.push_back(istack * (n + 1) + islice);
      elements.push_back((istack + 1) * (n + 1) + islice);
    }
  }
}
...
GLBezierSurface::GLBezierSurface(vec3 c[16], GLint d) : elementBuffer(GL_ELEMENT_ARRAY_BUFFER)
{
  ...
  GLBezierSurfaceBuilder(controls, vertices, elements).Build(division);
}
