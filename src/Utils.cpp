...
string Narrow(const wstring &wstr)
{
  ...
  UINT codepage = CP_ACP;  // system default
  int size_needed = WideCharToMultiByte(codepage, 0, wstr.data(), (int)wstr.size(), nullptr, 0, nullptr, nullptr);
  string result(size_needed, 0);
  WideCharToMultiByte(codepage, 0, wstr.data(), (int)wstr.size(), result.data(), size_needed, nullptr, nullptr);
  return result;
#else
...
wstring Widen(const string &str)
{
  ...
  // Generated by ChatGPT.
  UINT codepage = CP_ACP;  // system default
  int size_needed = MultiByteToWideChar(codepage, 0, str.data(), (int)str.size(), nullptr, 0);
  wstring result(size_needed, 0);
  MultiByteToWideChar(codepage, 0, str.data(), (int)str.size(), result.data(), size_needed);
  return result;
#else
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
